...
class Imagination:
    """
    Describes imagination operator of modal logic formula and it's semantics
    """
    ...
    def semantic(self, ks, world_to_test):
        stack = [world_to_test]  # Pilha para acompanhar os mundos a serem testados
        tested = set()  # Conjunto para rastrear os mundos já testados

        while stack:
            current_world = stack.pop()
            tested.add(current_world)

            for relation in ks.relations:
                if relation[0] == current_world:
                    next_world = relation[1]
                    if next_world not in tested:  # Verifica se o próximo mundo já foi testado
                        stack.append(next_world)
            
            if self.inner.semantic(ks, current_world):
                return True
    ...
    #         if relation[0] == world_to_test:
    #             result = result or self.inner.semantic(ks, relation[1])
    #             result = result or self.semantic(ks, relation[1]) # Recursive call
    #     return result

    def __eq__(self, other):
        return isinstance(other, Imagination) and self.inner == other.inner
